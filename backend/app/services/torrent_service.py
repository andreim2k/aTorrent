...
from app.core.cache import cached, torrent_cache
from app.core.monitoring import monitor_performance, monitor
...
class TorrentService:
    ...
    @monitor_performance("add_torrent")
    ...
    async def _save_torrent_to_database(
        ...
        try:
            ...
            # Use DatabaseManager to create record and get the ID
            with get_db_context() as db:
                db.add(db_torrent)
                db.commit()
    ...
    async def remove_torrent(self, torrent_id: int, delete_files: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get torrent from database with proper session context
            info_hash = None
            download_path = None
            torrent_name = None
            with get_db_context() as db:
            ...
            if delete_files and not removed_from_session and download_path and torrent_name:
                await self._manual_file_cleanup(download_path, torrent_name)
    ...
    async def _manual_file_cleanup(self, download_path: str, torrent_name: str) -> None:
        ...
        try:
            ...
            torrent_data_path = base_path / torrent_name
            ...
            if torrent_data_path.exists():
                ...
                logger.info(f"Manually deleted files for: {torrent_name}")

        except Exception as e:
            logger.error(f"Error manually deleting files: {e}")

    @monitor_performance("get_all_torrents_status")
    async def get_all_torrents_status(self) -> List[Dict[str, Any]]:
        """Get status of all torrents with improved data handling."""
        try:
            # Use proper session context to avoid detached instance errors
            with get_db_context() as db:
    ...
    def _update_torrent_from_handle(self, torrent) -> None:
        ...
        try:
            ...
            # Update torrent data using DatabaseManager to avoid session issues
            update_data = {
                ...
                "status": TorrentStatusMapper.map_status(status)
            }
            
            # Update total size if we didn't have it
            if torrent.total_size == 0 and hasattr(status, "total_wanted"):
                update_data["total_size"] = status.total_wanted
            
            # Use DatabaseManager to update safely
    ...
    @cached(ttl=30, cache_instance=torrent_cache)
