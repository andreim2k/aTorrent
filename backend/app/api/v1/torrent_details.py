    ...
    try:
        ...
        for i in range(file_storage.num_files()):
            ...
            if status.total_done > 0:
                ...
                try:
                    ...
                    if i < len(file_progresses):
                        file_progress = file_progresses[i] / file_size if file_size > 0 else 0.0
                except:
            ...
            try:
                ...
                priority_name = "high" if priority >= 4 else "normal" if priority >= 1 else "skip"
            except:
            ...
            if file_ext in ['.mkv', '.mp4', '.avi', '.mov', '.wmv', '.flv', '.webm', '.m4v']:
                file_type = "video"
            ...
            files.append({
                ...
                "progress": min(1.0, max(0.0, file_progress)),  # Clamp between 0 and 1
                "type": file_type,
        ...
        raise HTTPException(status_code=500, 
                            detail=f"Failed to get torrent content: {str(e)}")
...
def extract_movie_info_from_name(torrent_name: str) -> Dict[str, Any]:
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            title = re.sub(r'\b(PROPER|REPACK|INTERNAL|LIMITED|EXTENDED|' +
    ...
    title = re.sub(r'\b(720p|1080p|2160p|4K|BluRay|WEB-DL|HDTV|DVDRip|' +
                   r'BRRip|x264|x265|h264|h265).*', '', title, 
...
async def get_detailed_movie_info(movie_id: int, api_key: str, 
                                   movie_info: Dict, torrent_name: str) -> Dict:
    ...
    formatted_info = {
        ...
        "spoken_languages": [lang["english_name"] 
                           for lang in movie_details.get("spoken_languages", [])],
        ...
        "backdrop_path": (f"https://image.tmdb.org/t/p/w1280{movie_details['backdrop_path']}" 
    ...
    if "credits" in movie_details:
        ...
        # Find director(s)
        directors = [person["name"] for person in crew 
        ...
        key_jobs = ["Director", "Producer", "Executive Producer", "Writer", 
                   "Screenplay", "Director of Photography", "Original Music Composer"]
        formatted_info["crew"] = [
            {
            ...
            for person in crew if person.get("job") in key_jobs
        ][:15]  # Limit to 15 crew members
    ...
    if "videos" in movie_details:
        ...
        formatted_info["trailers"] = [
            ...
            for video in videos if video.get("type") in ["Trailer", "Teaser", "Clip"]
        ][:5]  # Top 5 videos
        ...
        formatted_info["similar_movies"] = [
            {
                ...
                "poster_path": (f"https://image.tmdb.org/t/p/w185{movie['poster_path']}" 
